...
def box2corners_th(box: torch.Tensor) -> torch.Tensor:
    ...
    x4 = torch.tensor([0.5, -0.5, -0.5, 0.5], dtype=w.dtype).unsqueeze(0).unsqueeze(0).to(box.device)  # (1,1,4)
    x4 = x4 * w  # (B, N, 4)
    # y4 = torch.FloatTensor([0.5, 0.5, -0.5, -0.5]).unsqueeze(0).unsqueeze(0).to(box.device)
    y4 = torch.tensor([0.5, 0.5, -0.5, -0.5], dtype=h.dtype).unsqueeze(0).unsqueeze(0).to(box.device)
    y4 = y4 * h  # (B, N, 4)
