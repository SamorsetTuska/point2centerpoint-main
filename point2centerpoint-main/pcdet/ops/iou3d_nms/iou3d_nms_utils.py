...
# add by HJH 0408
def sin_difference_pairwise(boxes_a, boxes_b):
    '''
    boxes1: N,1
    boxes2: M,1
    ...
    '''
    assert boxes_a.shape[1] == boxes_b.shape[1] == 1
    sin_dist = torch.cuda.FloatTensor(torch.Size((boxes_a.shape[0], boxes_b.shape[0]))).zero_()

    iou3d_nms_cuda.sin_difference_pairwise_gpu(boxes_a.contiguous(), boxes_b.contiguous(), sin_dist)
...
def boxes_iou3d_gpu(boxes_a, boxes_b):
    ...
    """
    assert boxes_a.shape[1] == boxes_b.shape[1] == 7
    # boxes_a[:, 3:6] = torch.clamp(boxes_a[:, 3:6], min=1e-2)
    ...
    # vol_a = (boxes_a[:, 3].clamp(min=1e-2) * boxes_a[:, 4].clamp(min=1e-2) * boxes_a[:, 5].clamp(min=1e-2)).view(-1, 1)
    vol_a = (boxes_a[:, 3] * boxes_a[:, 4] * boxes_a[:, 5]).view(-1, 1)
