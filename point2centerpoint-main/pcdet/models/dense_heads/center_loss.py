...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes):
        ...
        if self.use_focal:
            ...
            # class_loss = self.sigmoid_focal_iou_loss(pos_logits,
            #                                          pos_labels,
            #                                          neg_logits,
            #                                          neg_labels,
            #                                          alpha=self.focal_loss_alpha,
    ...
    def forward(self, outputs, targets):
        ...
        for b in range(bs):
            ...
            if self.use_focal:
                ...
                # neg_cost_class = (1 - alpha) * (out_prob * diou_match ** gamma) * (
                #     -(1 - out_prob * diou_match + 1e-6).log())
                # pos_cost_class = alpha * ((1 - out_prob * diou_match) ** gamma) * (
                #     -(out_prob * diou_match + 1e-6).log())
