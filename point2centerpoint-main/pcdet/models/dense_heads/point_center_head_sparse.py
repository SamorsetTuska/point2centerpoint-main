...
#
#         self.heatmap = spconv.SparseSequential(
#             spconv.SubMConv2d(num_features_in, feature_size, 3, padding=1, bias=False, indice_key='heatmap'),
#             nn.BatchNorm1d(feature_size),
#             nn.ReLU(),
#             spconv.SubMConv2d(feature_size, num_classes, 1, padding=0, bias=True, indice_key='heatmap')
#         )
#
#         if spconv.SubMConv2d in self._modules:
#             self._modules[spconv.SubMConv2d].apply(self.init_weights)
#         if nn.BatchNorm1d in self._modules:
#             self._modules[nn.BatchNorm1d].apply(self.init_weights)
#
        ...
        for cur_name in self.sep_head_dict:
            ...
            for k in range(num_conv - 1):
                fc_list.append(spconv.SparseSequential(
                    spconv.SparseConv2d(input_channels, input_channels, 3, padding=1, bias=use_bias,
                                        indice_key=cur_name),
                    nn.BatchNorm1d(input_channels),
                    nn.ReLU()
                ))
            fc_list.append(spconv.SparseConv2d(input_channels, output_channels, 3, padding=1, bias=True,
...
class SharedConvSparse(nn.Module):
    def __init__(self, num_features_in, feature_size):
        super(SharedConvSparse, self).__init__()
        self.act = nn.ReLU()
        self.conv0 = spconv.SparseSequential(
            nn.ReLU(),
            spconv.SubMConv2d(num_features_in, num_features_in // 2, kernel_size=1, stride=1, padding=0, bias=False,
                              indice_key='spconv_conv0'),
            nn.BatchNorm1d(num_features_in // 2),
            nn.ReLU(),
            spconv.SubMConv2d(num_features_in // 2, num_features_in // 2, kernel_size=3, stride=1, padding=1,
                              bias=False,
                              indice_key='spconv_conv0'),
            nn.BatchNorm1d(num_features_in // 2),
            nn.ReLU(),
            spconv.SubMConv2d(num_features_in // 2, num_features_in, kernel_size=1, stride=1, padding=0, bias=False,
                              indice_key='spconv_conv0'),
            nn.BatchNorm1d(num_features_in),
        )
        self.down1 = spconv.SparseSequential(
            spconv.SparseConv2d(num_features_in, 2 * num_features_in, kernel_size=3, stride=2, padding=1, bias=False,
                                indice_key='spconv_down1'),
            nn.BatchNorm1d(2 * num_features_in),
            nn.ReLU(),
        )
        self.conv1 = spconv.SparseSequential(
            spconv.SubMConv2d(2 * num_features_in, num_features_in // 2, kernel_size=1, stride=1, padding=0,
                              bias=False, indice_key='spconv_conv1'),
            nn.BatchNorm1d(num_features_in // 2),
            nn.ReLU(),
            spconv.SubMConv2d(num_features_in // 2, num_features_in // 2, kernel_size=3, stride=1, padding=1,
                              bias=False, indice_key='spconv_conv1'),
            nn.BatchNorm1d(num_features_in // 2),
            nn.ReLU(),
            spconv.SubMConv2d(num_features_in // 2, 2 * num_features_in, kernel_size=1, stride=1, padding=0,
                              bias=False, indice_key='spconv_conv1'),
            nn.BatchNorm1d(2 * num_features_in),
        )
        ...
        self.conv2 = spconv.SparseSequential(
            spconv.SparseConv2d(2 * num_features_in, 2 * num_features_in, kernel_size=3, stride=1, padding=1,
                                bias=False, indice_key='spconv_conv2'),
            nn.BatchNorm1d(2 * num_features_in),
            nn.ReLU(),
            spconv.SubMConv2d(2 * num_features_in, 2 * num_features_in, kernel_size=3, stride=1, padding=1,
                              bias=False, indice_key='spconv_conv2_'),
            nn.BatchNorm1d(2 * num_features_in),
            nn.ReLU(True)
        )

        self.down2 = spconv.SparseSequential(
            spconv.SparseConv2d(2 * num_features_in, 4 * num_features_in, kernel_size=3, stride=2, padding=1,
                                bias=False, indice_key='spconv_down2'),
            nn.BatchNorm1d(4 * num_features_in),
            nn.ReLU(),
        )

        self.conv3 = spconv.SparseSequential(
            spconv.SubMConv2d(4 * num_features_in, num_features_in, kernel_size=1, stride=1, padding=0, bias=False,
                              indice_key='spconv_conv3'),
            nn.BatchNorm1d(num_features_in),
            nn.ReLU(),
            spconv.SubMConv2d(num_features_in, num_features_in, kernel_size=3, stride=1, padding=1, bias=False,
                              indice_key='spconv_conv3'),
            nn.BatchNorm1d(num_features_in),
            nn.ReLU(),
            spconv.SubMConv2d(num_features_in, 4 * num_features_in, kernel_size=1, stride=1, padding=0, bias=False,
                              indice_key='spconv_conv3'),
            nn.BatchNorm1d(4 * num_features_in),
        )
        ...
        self.conv4 = spconv.SparseSequential(
            spconv.SparseConv2d(4 * num_features_in, 4 * num_features_in, kernel_size=3, stride=1, padding=1,
                                bias=False,
                                indice_key='spconv_conv4'),
            nn.BatchNorm1d(4 * num_features_in),
            nn.ReLU(),
            spconv.SubMConv2d(4 * num_features_in, 4 * num_features_in, kernel_size=3, stride=1, padding=1, bias=False,
                              indice_key='spconv_conv4_'),
            nn.BatchNorm1d(4 * num_features_in),
            nn.ReLU(True)
        )
        ...
        self.conv5 = spconv.SparseSequential(
            spconv.SubMConv2d(4 * num_features_in, feature_size, kernel_size=1, padding=0, bias=False,
                              indice_key='spconv_conv5'),
            nn.BatchNorm1d(feature_size),
            nn.ReLU()
        )

        # self.transconv = spconv.SparseSequential(
        #     spconv.SparseConvTranspose2d(4 * num_features_in, 2 * num_features_in, kernel_size=2, stride=2, padding=0,
        #                                  bias=False, indice_key='spconv_up1'),
        #     nn.BatchNorm1d(2 * num_features_in),
        #     nn.ReLU(),
        # )
        self.up1 = spconv.SparseSequential(
            spconv.SubMConv2d(4 * num_features_in, 2 * num_features_in, kernel_size=1, stride=1, padding=0,
                              bias=False, indice_key='spconv_up1'),
            nn.BatchNorm1d(2 * num_features_in),
            nn.ReLU(),
        )

        # self.transconv2 = spconv.SparseSequential(
        #     spconv.SparseConvTranspose2d(2 * num_features_in, num_features_in, kernel_size=2, stride=2, bias=False,
        #                                  indice_key='spconv_up2'),
        #     nn.BatchNorm1d(num_features_in),
        #     nn.ReLU())

        self.up2 = spconv.SparseSequential(
            spconv.SubMConv2d(2 * num_features_in, num_features_in, kernel_size=1, stride=1, padding=0,
                              bias=False, indice_key='spconv_up2'),
            nn.BatchNorm1d(num_features_in),
            nn.ReLU(),
        )
        self.down = spconv.SparseSequential(
            spconv.SparseConv2d(2 * num_features_in, feature_size, kernel_size=3, stride=2, padding=1, bias=False,
                                indice_key='down'),
            nn.BatchNorm1d(feature_size),
            nn.ReLU()
        )

        if spconv.SparseConv2d in self._modules:
            self._modules[spconv.SparseConv2d].apply(self.init_weights)
        if spconv.SubMConv2d in self._modules:
            self._modules[spconv.SubMConv2d].apply(self.init_weights)
        if nn.BatchNorm1d in self._modules:
            self._modules[nn.BatchNorm1d].apply(self.init_weights)
    ...
    def init_weights(self, m):
        if isinstance(m, spconv.SparseConv2d):
            nn.init.kaiming_normal_(m.weight.data, mode='fan_out', nonlinearity='relu')
            nn.init.constant_(m.bias.data, 0)
    ...
    def forward(self, x, training=True):
        ...
        # out = replace_feature(out, self.conv1(out).features + out.features)
        out = replace_feature(out, self.act(self.conv1(out).features + out.features))
        out = self.conv2(out)
        out_ = out
        # 残差模块降采样
        out = self.down2(out)
        # out = replace_feature(out, self.conv3(out).features + self.side_conv2(out).features)
        out = replace_feature(out, self.act(self.conv3(out).features + out.features))
        out = self.conv4(out)

        # out_ = replace_feature(out_, torch.cat([out_.features, self.transconv(out).features]))
        # out_.indices = torch.cat([out_.indices, self.transconv(out).indices])
        out_clone = self.up1(out)
        out_clone.indices[:, 1:] *= 2
        out_ = replace_feature(out_, torch.cat([out_.features, out_clone.features]))
        out_.indices = torch.cat([out_.indices, out_clone.indices])

        # bug
        out.indices[:, 1:] //= 2

        if training:
            out_clone_ = self.up2(out_)
            out_clone_.indices[:, 1:] *= 2
            x = replace_feature(x, torch.cat([x.features, out_clone_.features]))
            x.indices = torch.cat([x.indices, out_clone_.indices])
            # x = replace_feature(x, torch.cat([x.features, self.transconv2(out_).features]))
            # x.indices = torch.cat([x.indices, self.transconv2(out_).indices])
        ...
        # out = replace_feature(out, torch.cat([out.features, out_.features]))
        # out.indices = torch.cat([out.indices, out_.indices])

        return x, out_, out


class Point2CenterHeadSparse(nn.Module):
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_size, point_cloud_range, voxel_size,
        ...
        self.shared_conv_sparse = SharedConvSparse(num_features_in=self.input_channels,
    ...
    def forward(self, data_dict):
        ...
        if not self.training or self.predict_boxes_when_training or self.test_aux_head:
            spatial_features_2d, x_aux, x = self.shared_conv_sparse(spatial_features_2d, training=False)
        else:
            spatial_features_2d, x_aux, x = self.shared_conv_sparse(spatial_features_2d, training=True)
        ...
        if self.training:
            ...
            if self.use_gt_mask:
                if data_dict['cur_epoch'] <= 10:
                    ...
                    heatmaps_ = self.assign_targets(
                        data_dict['gt_boxes'], feature_map_size=x.dense().size()[2:],
