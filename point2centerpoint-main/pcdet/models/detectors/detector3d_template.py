    ...
    def generate_recall_record(box_preds, recall_dict, batch_index, data_dict=None, thresh_list=None):
        ...
        if cur_gt.shape[0] > 0:
            if box_preds.shape[0] > 0:
                ...
                ## compute L2 distance between box_preds' and cur_gt's centers
                # iou3d_rcnn = torch.norm(box_preds[:, :3].unsqueeze(1) - cur_gt[:, :3].unsqueeze(0), p=2, dim=2)
            else:
            ...
            for cur_thresh in thresh_list:
                if iou3d_rcnn.shape[0] == 0:
                    ...
                    # rcnn_recalled = (iou3d_rcnn.min(dim=0)[0] < cur_thresh).sum().item()
